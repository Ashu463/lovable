          ...
          onEvent: ({ runEvents: event }) => {
            switch (event.type) {
              ...
              case "designs_generating":
                ...
                return;
              default: {
                // The DAG tree (DagView) is the live view of coder/uiExpert work
                // — which agent is on which task, blinking while it runs. Those
                // planned tasks have DAG nodes, so their subagent events feed the
                // tree, not the text feed. The debugger and tester, though, are
                // spawned dynamically by the merge gate with no planned taskId,
                // so they never appear in the DAG — surface their start/finish as
                // activity-feed lines instead (skip per-iteration progress ticks
                // to avoid spam).
                const suppress =
                  event.type === "subagent_progress" ||
                  ((event.type === "subagent_started" || event.type === "subagent_completed") &&
                    (event.agent === "coder" || event.agent === "uiExpert"));
                if (!suppress) pushMessage("system", describeEvent(event), "progress");
                setState((prev) =>
